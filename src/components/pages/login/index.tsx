...
export default function Login() {
  ...
  const handlesubmit = (e: React.FormEvent) => {
    ...
    // Pegando usuário mockado
    const user = mockUser.find(
      (u) => u.email === email && u.password === password,
    );

    if (user) {
      // Remove a senha antes de salvar no estado global
      ...
      <div className="w-full max-w-md bg-[#111] border border-[#2a2a2a] rounded-xl p-6 sm:p-8 relative overflow-hidden">
        <div className="relative z-10">
          ...
          <form onSubmit={handlesubmit} className="space-y-4 sm:space-y-5">
            {error && (
              <div className="p-3 bg-[#2b1111] border border-[#e57373] text-[#e57373] rounded-xl text-sm text-center font-medium">
                {error}
              </div>
            )}
